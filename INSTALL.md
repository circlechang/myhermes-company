...
右上角出現「有新版 vX.Y.Z」時點它，或直接開「管理 → 版本」，按 **立即更新到 vX.Y.Z**：

1. 先下載新版 wheel 並驗證（檔名、版號、是不是完整的 zip、裡面是不是這個產品）。
   **驗不過就直接回報錯誤，完全不會動到你現在能用的版本。**
2. 驗過才開始換：伺服器優雅關掉自己（執行中的工作流會被放下、等待中的閘門不會被取消），
   由一支獨立的更新器裝好新版、再用原本的 port 把服務起回來。
3. 畫面會停在「下載中／安裝中／重啟中」，中間**大約離線 30 秒**——這是正常的，
   不要關瀏覽器。回來之後顯示「已更新到 vX.Y.Z」，按「重新整理頁面」即可。

- 只有 owner 看得到這個按鈕。
- **開發安裝（`pip install -e` 或直接跑原始碼）不給一鍵更新**，會改提示你 `git pull` 後重啟。
- 萬一裝失敗，服務會用**舊版**起回來（pip 升級失敗不會動到已安裝的版本），
  畫面會給你失敗原因與 log 路徑（`~/.myhermescompany/logs/update-*.log`）。
  重新整理後在同一頁也還看得到進度／結果（狀態存在 `~/.myhermescompany/update-status.json`）。
