  ...
  return (
    ...
    >
      <div className="flex min-w-0 items-center gap-2">
        ...
        <span className="shrink-0 text-xs tabular-nums text-zinc-600 dark:text-zinc-400" data-testid={`station-${n.id}-seq`}>{t('wf.station.seq', { n: station.seq })}</span>
      ...
      <Row label={t('wf.station.output')} testId={`station-${n.id}-output`}>
        {docCount > 0 && (
          <span className="badge mr-1 bg-sky-100 text-2xs text-sky-800 dark:bg-sky-900/40 dark:text-sky-200" data-testid={`station-${n.id}-docs`}>
      ...
      {state?.status === 'running' && state.streaming && (
        <pre className="max-h-24 overflow-auto whitespace-pre-wrap rounded bg-zinc-100 p-1.5 text-xs dark:bg-zinc-800" data-testid={`station-${n.id}-stream`}>{state.streaming.slice(-600)}</pre>
      )}
      ...
      {approval && (
        <div className="space-y-1 rounded border border-amber-300 bg-amber-50 p-2 dark:border-amber-700 dark:bg-amber-950/30" data-testid={`station-${n.id}-approval`}>
          <pre className="max-h-40 overflow-auto whitespace-pre-wrap text-xs">{approval.payload}</pre>
