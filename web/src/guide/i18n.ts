  ...
  guide: {
    tour: {
      ...
      steps: {
        ...
        workflows: { title: '工作流把員工串起來', body: '在畫布上放節點並連線：熱點 → 選題 → 審批閘門 → 文案。跑到閘門會停下來等你核准，通過後才花下一段錢。' },
    ...
    empty: {
      ...
      workflows: { title: '還沒有工作流', body: '工作流把多位 AI 員工串成一條流水線，中間可以放審批閘門。先載入範例看它怎麼跑，再改成自己的。', action: '載入範例：熱點→選題→閘門→文案', create: '＋ 自己新建' },
  ...
  guide: {
    tour: {
      ...
      steps: {
        ...
        workflows: { title: 'Workflows chain agents', body: 'Place nodes and connect: hot topics → pick → approval gate → copy. The gate pauses for your approval before spending more.' },
    ...
    empty: {
      ...
      workflows: { title: 'No workflows yet', body: 'A workflow chains agents into a pipeline with optional approval gates. Load the example first, then edit it.', action: 'Load example: hot → pick → gate → copy', create: '+ Create your own' },
